<?php
if ( ! class_exists( 'AERIS_WIDGET_MENU' ) ) {

	class AERIS_WIDGET_MENU {
		...
		public function sidebar() {
			register_sidebar( array(
				...
				'id' => AERIS_WIDGET_MENU_PREFIX,
				"before_widget" => '<div id="%1$s" class="' . AERIS_WIDGET_MENU_PREFIX . '_widget %2$s">',
				"after_widget" => '</div>',
				'description' => __( 'Widgets in this area will be shown on the edit menu screen.', $this->domain ),
				'before_title' => '<span class="' . AERIS_WIDGET_MENU_PREFIX . '_title">',
				'after_title' => '</span>'
			) );
		}
		...
		public function meta_box() {
			...
			if ( empty( $wp_registered_sidebars[ AERIS_WIDGET_MENU_PREFIX ] ) || empty( $sidebars_widgets[ AERIS_WIDGET_MENU_PREFIX ] ) || ! is_array( $sidebars_widgets[ AERIS_WIDGET_MENU_PREFIX ] ) ) {
				 ...
				 */
				$no_widgets_output = apply_filters( 'aeris_widget_menu_no_widgets_message', $no_widgets_output );
				...
				foreach ( ( array ) $sidebars_widgets[ AERIS_WIDGET_MENU_PREFIX ] as $id ) {
					...
					$output .= '<input type="hidden" class="menu-item-type" name="menu-item['
						. $_nav_menu_placeholder . '][menu-item-type]" value="' . AERIS_WIDGET_MENU_PREFIX . '" />';
			...
			// submit button
			?>
			<div class="aeris_widget_menudiv" id="aeris_widget_menudiv">
				<?php echo $output; ?>
			...
			foreach ( ( array ) $_POST[ 'menu-item' ] as $menu_item_data ) {
				...
				if (
					...
					! empty( $menu_item_data[ 'menu-item-object-id' ]) &&
						AERIS_WIDGET_MENU_PREFIX != $menu_item_data[ 'menu-item-type' ] // this is the second extra line
				) 
					{
		...
		public function enqueue( $hook ) {
			...
			wp_enqueue_script(
					AERIS_WIDGET_MENU_PREFIX, AERIS_WIDGET_MENU_URL . "/assets/js/aeris-widget-menu.js", array( 'nav-menu' ), $WORDPRESS_SOCIAL_LOGIN_VERSION
			);
		}
		...
		function label( $item ) {
			...
			if ( $item->object === AERIS_WIDGET_MENU_PREFIX ) {
