...
if ( ! class_exists( 'AERIS_WIDGET_MENU_Walker' ) ) {
	...
	class AERIS_WIDGET_MENU_Walker {
		...
		public function start_el( $item_output, $item, $depth, $args ) {
			...
			$sidebar = array_merge(
				// our sidebar is at the index 'aeris_widget_menu'
				$wp_registered_sidebars[ AERIS_WIDGET_MENU_PREFIX ],
			 ...
			 * }
			 */
			$params = apply_filters( 'aeris_widget_menu_widget_params', $params );

			$aeris_widget_menu_widget = $wp_registered_widgets[ $id ];
			 ...
			 * @see dynamic_sidebar()
			 *
			 * @param array $aeris_widget_menu_widget {
			 ...
			 * }
			 */
			do_action( 'aeris_widget_menu_pre_callback', $aeris_widget_menu_widget );

			// set up the wrapper class
			$wrapper_class = AERIS_WIDGET_MENU_PREFIX . '_wrap';
